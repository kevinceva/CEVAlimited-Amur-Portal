...
public class ProductDetailsServlet extends HttpServlet {
	...
	Logger logger = Logger.getLogger(ProductDetailsServlet.class);
	...
	public JSONObject fetchClientDetails(int totalRecords, HttpServletRequest request) throws SQLException, ClassNotFoundException {
		...
		if (GLOBAL_SEARCH_TERM != null && GLOBAL_SEARCH_TERM.length()>0) {
			...
			query ="SELECT count(*) from PRODUCT_MASTER where PRD_NAME like ?  or PRD_DESC like ? ";
	...
	/*		 query ="select to_char(DATE_CREATED,'HH:MI AM Month DD, YYYY'),POLICY_ID, AMOUNT, STATUS,AIRTIME_ID from "
			...
			query =" SELECT NAME, pdsc, MDL, PRICE, disc, qty  from ( SELECT NAME, pdsc, MDL, PRICE, disc, qty,  rownum rn from ( SELECT PRD_NAME NAME, PRD_DESC pdsc, "
			+ " MODEL_BRAND_NO MDL, PRICE , decode(DISCOUNT_TYPE,'P',DISCOUNT||'%','Ksh.'||DISCOUNT) disc, QUANTITY qty from  PRODUCT_MASTER  "
			+ " where PRD_NAME like ?  or PRD_DESC like ?  "
			+ " order by PRD_NAME )) "
			+ " where rn between " + intiaCnt + " and " + (INITIAL+RECORD_SIZE) ;
			...
			sql =" SELECT NAME, pdsc, MDL, PRICE, disc, qty  from ( SELECT NAME, pdsc, MDL, PRICE, disc, qty,  rownum rn from ( SELECT PRD_NAME NAME, PRD_DESC pdsc, "
			+ " MODEL_BRAND_NO MDL, PRICE , decode(DISCOUNT_TYPE,'P',DISCOUNT||'%','Ksh.'||DISCOUNT) disc, QUANTITY qty from  PRODUCT_MASTER  "
			+ " order by PRD_NAME )) "
			+ " where rn between " + intiaCnt + " and " + (INITIAL+RECORD_SIZE) ;
