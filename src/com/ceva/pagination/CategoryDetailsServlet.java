...
public class CategoryDetailsServlet extends HttpServlet {
	...
	Logger logger = Logger.getLogger(CategoryDetailsServlet.class);
	...
	public JSONObject fetchClientDetails(int totalRecords, HttpServletRequest request) throws SQLException, ClassNotFoundException {
		...
		if (GLOBAL_SEARCH_TERM != null && GLOBAL_SEARCH_TERM.length()>0) {
			...
			query ="SELECT count(*) from CATEGORY_MASTER cm, SUB_CATEGORY_MASTER sc "
			+ " WHERE cm.CATEGORY_ID=sc.CATEGORY_ID and (cm.CATEGORY_ID like ? or cm.CATEGORY_NAME like ? or cm.CATEGORY_DESC like ? or sc.SUB_CATEGORY_DESC like ? or sc.SUB_CATEGORY_NAME like ? )";
	...
	/*		 query ="select to_char(DATE_CREATED,'HH:MI AM Month DD, YYYY'),POLICY_ID, AMOUNT, STATUS,AIRTIME_ID from "
			...
			query =" SELECT NAME, SNAME, CDT  from ( SELECT NAME, SNAME, CDT, rownum rn from ( SELECT cm.CATEGORY_NAME NAME, sc.SUB_CATEGORY_NAME SNAME, "
			+ " to_char(cm.created_date,'dd/mm/yyyy') CDT from  CATEGORY_MASTER cm, SUB_CATEGORY_MASTER sc "
			+ " where cm.CATEGORY_ID=sc.CATEGORY_ID and (cm.CATEGORY_ID like ? or cm.CATEGORY_NAME like ? or cm.CATEGORY_DESC like ? or sc.SUB_CATEGORY_DESC like ? or sc.SUB_CATEGORY_NAME like ? ) "
...
/*				sql ="select to_char(DATE_CREATED,'HH:MI AM Month DD, YYYY'),POLICY_ID, AMOUNT, STATUS,AIRTIME_ID, mpid from "
			...
			sql =" SELECT NAME, SNAME, CDT  from ( SELECT NAME, SNAME, CDT, rownum rn from ( SELECT cm.CATEGORY_NAME NAME, sc.SUB_CATEGORY_NAME SNAME,  "
			+ " to_char(cm.created_date,'dd/mm/yyyy') CDT from  CATEGORY_MASTER cm, SUB_CATEGORY_MASTER sc "
			+ " where cm.CATEGORY_ID=sc.CATEGORY_ID  "
			+ " order by cm.created_date desc )) "
			+ " where rn between " + intiaCnt + " and " + (INITIAL+RECORD_SIZE) ;
	...
	public int getTotalRecordCount() throws SQLException {
		...
		sql ="SELECT count(*) from CATEGORY_MASTER cm, SUB_CATEGORY_MASTER sc where cm.CATEGORY_ID=sc.CATEGORY_ID ";
