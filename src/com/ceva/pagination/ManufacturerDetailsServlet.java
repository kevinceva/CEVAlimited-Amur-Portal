...
public class ManufacturerDetailsServlet extends HttpServlet {
	...
	Logger logger = Logger.getLogger(ManufacturerDetailsServlet.class);
	...
	public JSONObject fetchClientDetails(int totalRecords, HttpServletRequest request) throws SQLException, ClassNotFoundException {
		...
		if (GLOBAL_SEARCH_TERM != null && GLOBAL_SEARCH_TERM.length()>0) {
			...
			query ="SELECT count(*) from MANUFACTURER_MASTER "
			+ " WHERE MANUFACTURER_NAME like ? or MANUFACTURER_CONTACT like ? or MANUFACTURER_SECOND_CONTACT like ? ";
	...
	/*		 query ="select to_char(DATE_CREATED,'HH:MI AM Month DD, YYYY'),POLICY_ID, AMOUNT, STATUS,AIRTIME_ID from "
			...
			query =" SELECT NAME, pcnt, scnt  from ( SELECT NAME, pcnt, scnt, rownum rn from ( SELECT MANUFACTURER_NAME NAME, MANUFACTURER_CONTACT pcnt, "
			+ " MANUFACTURER_SECOND_CONTACT scnt from  MANUFACTURER_MASTER  "
			+ " where  MANUFACTURER_NAME like ? or MANUFACTURER_CONTACT like ? or MANUFACTURER_SECOND_CONTACT like ? "
...
/*				sql ="select to_char(DATE_CREATED,'HH:MI AM Month DD, YYYY'),POLICY_ID, AMOUNT, STATUS,AIRTIME_ID, mpid from "
			...
			sql =" SELECT NAME, pcnt, scnt  from ( SELECT NAME, pcnt, scnt, rownum rn from ( SELECT MANUFACTURER_NAME NAME, MANUFACTURER_CONTACT pcnt, "
			+ " MANUFACTURER_SECOND_CONTACT scnt from  MANUFACTURER_MASTER  "
			+ " order by MANUFACTURER_NAME )) "
			+ " where rn between " + intiaCnt + " and " + (INITIAL+RECORD_SIZE) ;
	...
	public int getTotalRecordCount() throws SQLException {
		...
		sql ="SELECT count(*) from MANUFACTURER_MASTER ";
