...
public class CICSelfRegAgentsServlet extends HttpServlet {
	...
	Logger logger = Logger.getLogger(CICSelfRegAgentsServlet.class);
	...
	public JSONObject getLiveTransactionDetails(int totalRecords, HttpServletRequest request) throws SQLException, ClassNotFoundException {
		...
		if (GLOBAL_SEARCH_TERM != null && GLOBAL_SEARCH_TERM.length()>0) {
			...
			query ="SELECT count(*) FROM CIC_AGENT_MASTER TL WHERE TL.status='C' and (TL.AGENT_MOBILE_NUMBER like ? or TL.AGENT_NAME like ? ) ";
			...
			query = " select CD, MOB, NAME, EMAIL from ( select CD, MOB, NAME, EMAIL, rownum rn from "
			 		+ " ( select to_char(TL.CREATED_DATE,'HH:MI AM Month DD, YYYY') CD, TL.AGENT_MOBILE_NUMBER MOB, TL.AGENT_NAME NAME, TL.EMAIL EMAIL "
		    		+ " from CIC_AGENT_MASTER  TL "
		    		+ " where TL.STATUS='C' and (TL.AGENT_MOBILE_NUMBER like ? or TL.AGENT_NAME like ?)  order by TL.CREATED_DATE desc ) ) "
			...
			while (rs.next()) {
				...
				uId1="Acpt"+"$"+rs.getString(2);
				uId2="Rjct"+"$"+rs.getString(2);
				ja.put(i);
			...
			PreparedStatement stmt = null;
				...
				sql = " select CD, MOB, NAME, EMAIL from ( select CD, MOB, NAME, EMAIL, rownum rn from  "
				 		+ " (  select to_char(TL.CREATED_DATE,'HH:MI AM Month DD, YYYY') CD, TL.AGENT_MOBILE_NUMBER MOB, TL.AGENT_NAME NAME, TL.EMAIL EMAIL "
			    		+ " from CIC_AGENT_MASTER  TL  	 where TL.STATUS='C'  order by TL.CREATED_DATE desc ) ) "
			    		+ " where rn between " + intiaCnt + " and " + (INITIAL+RECORD_SIZE) ;
	...
	public int getTotalRecordCount() throws SQLException {
		...
		PreparedStatement stmt = null;
			...
			sql ="select count(*) from CIC_AGENT_MASTER where status='C' ";
