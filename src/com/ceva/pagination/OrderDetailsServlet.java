...
public class OrderDetailsServlet extends HttpServlet {
	...
	public JSONObject fetchClientDetails(int totalRecords, HttpServletRequest request) throws SQLException, ClassNotFoundException {
		...
		if (GLOBAL_SEARCH_TERM != null && GLOBAL_SEARCH_TERM.length()>0) {
			...
			query ="SELECT count(*) from ORDER_MASTER om, customer_master cm, ORDER_STATUS_MASTER osm "
			+ " WHERE om.ORDER_STATUS=osm.id and om.cin=cm.cin and "
			+ " ( om.ORDER_ID like ? or cm.fname like ? or cm.lname like ? osm.status like ? ) ";
	...
	/*		 query ="select to_char(DATE_CREATED,'HH:MI AM Month DD, YYYY'),POLICY_ID, AMOUNT, STATUS,AIRTIME_ID from "
			...
			query =" SELECT oid, name, stat, amt, odt  from ( SELECT oid, name, stat, amt, odt, rownum rn from ( SELECT om.ORDER_ID oid, cm.fname||' '||cm.lname name, "
			+ " osm.status stat, om.txn_amt amt, om.ORDERED_DATE odt  "
			+ " from order_master om, order_status_master osm, customer_master cm "
			+ " WHERE om.ORDER_STATUS=osm.id and om.cin=cm.cin and "
			+ " ( om.ORDER_ID like ? or cm.fname like ? or cm.lname like ? osm.status like ? ) "
...
/*				sql ="select to_char(DATE_CREATED,'HH:MI AM Month DD, YYYY'),POLICY_ID, AMOUNT, STATUS,AIRTIME_ID, mpid from "
			...
			sql =" SELECT oid, name, stat, amt, odt  from ( SELECT oid, name, stat, amt, odt, rownum rn from ( SELECT om.ORDER_ID oid, cm.fname||' '||cm.lname name, "
			+ " osm.status stat, om.txn_amt amt, om.ORDERED_DATE odt "
			+ " from order_master om, order_status_master osm, customer_master cm "
			+ " WHERE om.ORDER_STATUS=osm.id and om.cin=cm.cin  "
			+ " order by om.ORDER_ID desc )) "
			+ " where rn between " + intiaCnt + " and " + (INITIAL+RECORD_SIZE) ;
	...
	public int getTotalRecordCount() throws SQLException {
		...
		sql ="SELECT count(*) from order_master ";
