...
import com.ceva.base.common.dao.SMSFileUploadDAO;
...
public class SMSFileUploadAction extends ActionSupport {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	Logger logger = Logger.getLogger(SMSFileUploadAction.class);
...
/*	public String getMerchantDashBoard(){
	...
	public String execute()
	{
		{		
			...
			logger.debug("##############################  SMS File Upload Sart ##################");
			...
			SMSFileUploadDAO smsFileUploadDAO = null;
			...
			try{
				...
				SMSFileUploadDAO finalobj = new SMSFileUploadDAO();
				...
				if (!fileName.isEmpty()) {
					...
					while (rowIterator1.hasNext()) {
						...
						if(row.getRowNum()==0)
						{
							...
							if(finalstatus)
							{
								...
								msgText=getCellData(row, 1,"STR");
								
								if(!mobnos.contains(mobNo)){
									mobnos.add(mobNo);
								}
								else{
									finalStatus=false;
									logger.info("duplicate mobNo no found..:"+mobNo);
									errorresarayJSON = getJsonObject("Duplicate Mobile Number Found :"+mobNo, exlrownum, errorresarayJSON);
								}							
								

							logger.info("Row Data [mobNo=" + mobNo + ", msgText=" + msgText + "]");
							...
							if(StringUtil.isNullOrEmpty(msgText)){						
									finalStatus=false;
									remarks="Please Enter Message Text";
									errorresarayJSON= getJsonObject(msgText, exlrownum, errorresarayJSON);							
							}					

							reqJSON.put("mobNo", mobNo);
							reqJSON.put("msgText", msgText);					
					...
					if(finalStatus)
					{
						...
						String mapStatus =finalobj.initiateSMSDet(finalData,userId,fileName); //Need To do Chnage
						logger.debug("After Saved All Excel topup Data into [SMS Upload] Table mapStatus["+mapStatus+"]");
						...
						{	
							...
							logger.info("SMS Upload Action Final Response Data ["+finalerrrespjson+"]");
						...
						else
						{
							...
							errorresJSON.put("uploadErrors", "SMS Upload FAILED DUE TO INTERNAL ERROR");
					...
					logger.debug("############################## SMS File Upload End ##################");
