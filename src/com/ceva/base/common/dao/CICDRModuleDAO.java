...
public class CICDRModuleDAO
{
	private Logger logger = Logger.getLogger(CICDRModuleDAO.class);
	...
	public ResponseDTO getClientDetails(RequestDTO requestDTO)
	{
		...
		this.logger.debug("Inside [CICDRModuleDAO][getClientDetails].. ");
	...
	{
		Connection connection = null;
		this.logger.debug("Inside [CICDRModuleDAO][getBeneficiaryDetails].. ");
	...
	{
		Connection connection = null;
		this.logger.debug("Inside [CICDRModuleDAO][getRequestAsResponse].. ");
	...
	{
		Connection connection = null;
		this.logger.debug("Inside [CICDRModuleDAO][getPurchaseDetails].. ");
	...
	{
		Connection connection = null;
		this.logger.debug("Inside [CICDRModuleDAO][getScenarios].. ");
	...
	{
		Connection connection = null;
		this.logger.debug("Inside [CICDRModuleDAO][getRaisedCases].. ");
	...
	{
		Connection connection = null;
		this.logger.debug("Inside [CICDRModuleDAO][getRaisedCasedetails].. ");
	...
	{
		Connection connection = null;
		this.logger.debug("Inside [CICDRModuleDAO][submitCompAction].. ");
	...
	{
		Connection connection = null;
		this.logger.debug("Inside [CICDRModuleDAO][getFailedTxns].. ");
	...
	{
		Connection connection = null;
		this.logger.debug("Inside [CICDRModuleDAO][getForceTopUpDet].. ");
	...
	public ResponseDTO drForceTopUp(RequestDTO requestDTO)
	{
		Connection connection = null;
		...
		String merchantQry = "select fname, lname from cic_clients where mobile_number=? ";
		String valMpesaRefno = "select count(*) from cic_drtopup_det where policy_id=? and mpesa_refno=?";
		String insDRTopUpQry = "insert into cic_drtopup_det (ref_no,policy_id,amount,bonus,mpesa_refno,status,MPESA_TXN_DATE,remarks,CREATED_BY,CREATED_DATE) "
				+ " values ('DRTOPUP'||seq_drtopup.nextval,?,?,?,?,?,to_date(?,'DD/MM/YYYY'),?,?,sysdate)";
		String updDRTopUpQry = "update cic_drtopup_det set status=? where policy_id=? and mpesa_refno=?";
		try
		{
			this.responseDTO = new ResponseDTO();
			...
			String famt = requestJSON.getString("amt");
			String bonus = requestJSON.getString("bonus");
			String remarks = requestJSON.getString("remarks");
			String txndt = requestJSON.getString("txndt");
			String refno = requestJSON.getString("refno");
			String mkrid = requestJSON.getString("mkrid");
			String fname="";
			String lname="";
			
			valPstmt = connection.prepareStatement(valMpesaRefno);
			valPstmt.setString(1, mob);
			valPstmt.setString(2, refno);
			valRS = valPstmt.executeQuery(); 
			
			if (valRS.next())
			{
				refno_cnt=valRS.getInt(1);
			}

			if (refno_cnt>0)
			{
				json.put("result", "Request Already Raised For This Reference Number.");
			}
			else
			{
				insPstmt = connection.prepareStatement(insDRTopUpQry);
				insPstmt.setString(1,mob );
				insPstmt.setString(2,famt );
				insPstmt.setString(3,bonus );
				insPstmt.setString(4,refno );
				insPstmt.setString(5,"INITIATED" );
				insPstmt.setString(6,txndt );
				insPstmt.setString(7,remarks );
				insPstmt.setString(8,mkrid );
				int insrt=insPstmt.executeUpdate();
				
				this.logger.debug("drtopup ins cnt ["+insrt+"]");
				...
				if (merchantRS.next())
				{
					...
					lname= merchantRS.getString(2);
				}
	
				this.logger.debug("DR TopUp URL Params - mob ["
						+ mob + "] amt["
				...
				this.logger.debug("Response Msg:"+responseMsg);
				...
				if ("00".equalsIgnoreCase(responseCode))
				{
					...
					updPstmt.setString(1,"SUCCESS" );
					updPstmt.setString(2,mob );
					updPstmt.setString(3,refno );
					updCnt=updPstmt.executeUpdate();	
					
					this.logger.debug("update successs status ["+updCnt+"] responseMsg ["+responseMsg+"]");
				}else
				{
					updPstmt = connection.prepareStatement(updDRTopUpQry);
					updPstmt.setString(1,"FAILED" );
					updPstmt.setString(2,mob );
					updPstmt.setString(3,refno );
					updCnt=updPstmt.executeUpdate();	
					
					this.logger.debug("update status ["+updCnt+"] responseMsg ["+responseMsg+"]");
				}
	...
	{
		Connection connection = null;
		this.logger.debug("Inside [CICDRModuleDAO][getTwoWaysSMSdetails].. ");
	...
	{
		Connection connection = null;
		this.logger.debug("Inside [CICDRModuleDAO][submitSMSAction].. ");
	...
	public String initiateDRTopUp(JSONArray finalData, String userId, String fileName){
		...
		String merchantQry = "select fname, lname from cic_clients where mobile_number=? ";
		String valMpesaRefno = "select count(*) from cic_drtopup_det where mpesa_refno=?";
		String insDRTopUpQry = "insert into cic_drtopup_det (ref_no,policy_id,amount,bonus,mpesa_refno,status,CREATED_BY,CREATED_DATE) "
				+ " values ('DRTOPUP'||seq_drtopup.nextval,?,?,?,?,?,?,sysdate)";
		String updDRTopUpQry = "update cic_drtopup_det set status=? where policy_id=? and mpesa_refno=?";
		try
		{
			...
			logger.info("vald refno Query Is ["+valMpesaRefno+"] ");
			logger.info("ins dr top up Query Is ["+insDRTopUpQry+"] ");
			logger.info("upd dr top up Query Is ["+updDRTopUpQry+"] ");
			...
			for (int i = 0; i < finalData.size(); i++) {
				...
				String amt = (String) jres.get("amount");
				String bonus = (String) jres.get("bonus");
				String refno = (String) jres.get("refno");
				String mkrid = userId;
				
				logger.debug(" Input values mob["+mob+"] amt["+amt+"] bonus["+bonus+"] refno["+refno+"]");
				String famt = Integer.toString((Integer.parseInt(amt) + (Integer.parseInt(amt) * (Integer.parseInt(bonus)/(100)))));
				
				valPstmt.setString(1, refno);
				valRS = valPstmt.executeQuery(); 
				...
				else
				{
					...
					insPstmt.setString(4,refno);
					insPstmt.setString(5,"INITIATED" );
					insPstmt.setString(6,userId );
					int insrt=insPstmt.executeUpdate();
					
					this.logger.debug("Loop ["+i+"] drtopup ins cnt ["+insrt+"]");
