...
public class CevaPowerAdminDAO_curr_prd_ver {

	private Logger logger = Logger.getLogger(CevaPowerAdminDAO_curr_prd_ver.class);
	...
	public ResponseDTO dashboardUsers(RequestDTO requestDTO) {
		...
		String merchantQry = "SELECT UG.GROUP_ID,"
				...
				+ "UG.GROUP_NAME,"
				+ "ULC.MAKER_ID,"
				+ "to_char(ULC.MAKER_DTTM,'DD-MON-YYYY HH24:MI:SS'), "
				+ "UG.MAKER_ID,"
				+ "to_char(UG.MAKER_DTTM,'DD-MON-YYYY HH24:MI:SS') "
		...
		try {
			...
			while (merchantRS.next()) {
				...
				json.put("USER_MAKER_ID", merchantRS.getString(4));
				json.put("USER_MAKER_DATE", merchantRS.getString(5));
				json.put("GROUP_MAKER_ID", merchantRS.getString(6));
				json.put("GROUP_MAKER_DATE", merchantRS.getString(7));
		...
		try {
			...
			for (String getGrps : groupsArray) {
				...
				try {
					...
					logger.debug("Got SQLexception in loop [" + e.getMessage()
							+ "]");
				} catch (Exception e) {
	...
	public ResponseDTO getAdminCraeteInfo(RequestDTO requestDTO) {
		...
		try {
			...
			resultJson = new net.sf.json.JSONObject();
			locationJSONArray = new net.sf.json.JSONArray();

			connection = DBConnector.getConnection();
			logger.debug(Boolean
					.valueOf("Connection is null [" + connection == null + "]"));
			...
			while (locationRS.next()) {
				json.put("key", locationRS.getString(2));
				json.put("val", locationRS.getString(1));
				locationJSONArray.add(json);
				json.clear();
			}
			resultJson.put("LOCATION_LIST", locationJSONArray);
			locationDataMap.put("LocationInfo", resultJson);
