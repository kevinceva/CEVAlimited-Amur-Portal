...
public class CICReportDetailsDAO {

	private Logger logger = Logger.getLogger(CICReportDetailsDAO.class);
...
/*			genQuery = "Select  CLIENT_ID,FNAME||' '||MNAME||' '||LNAME from CIC_CLIENTS where status='SUCCESS' ";
			...
			logger.debug("Client JSONArray [" + jsonArray + "]");

			resultJson.put("cid", jsonArray);
...
/*			genQuery = "SELECT RUG.REPORT_ID,(select REPORT_DESCRIPTION FROM REPORT_DETAILS WHERE REPORT_ID=RUG.REPORT_ID) "
			...
			genQuery = "SELECT REPORT_ID, REPORT_DESCRIPTION FROM REPORT_DETAILS order by REPORT_DESCRIPTION";
	...
	public ResponseDTO getClientPolicyNumbers(RequestDTO requestDTO) {

		logger.debug("Inside GetUserAssignedReports... ");

		String userQry = "SELECT POLICY_NUMBER FROM CIC_POLICY WHERE  CLIENT_ID= ? ";
		...
		try {
			...
			userPstmt = connection.prepareStatement(userQry);
			userPstmt.setString(1, requestJSON.getString("cid"));
			userRS = userPstmt.executeQuery();
			json = new JSONObject();

			while (userRS.next()) {

				json.put("PNO", userRS.getString(1));
				userJSONArray.add(json);
				json.clear();
			}
			...
			responseJSON.put("POLICY_NOS", userJSONArray);

			logger.debug("POLICY_NOS :::" + requestJSON.getString("POLICY_NOS"));
			userJSONArray.clear();
