...
$(document).ready(function () {
	...
	$.each(json, function(i, v) {
		...
		if(status=="Query Pending")
			status='<a href="#" id="QR-PEN">Queries Sent For Approval</a>';
		else if(status=="Query Completed")
			status='<a href="#" id="QC-COM">Queries Approved</a>';/* 
			else if(status=="Query Approved")
				status='<a href="#" id="QA-APP">Queries Sent For Approval</a>';	 */		
	...
	$('#sms-details').on('click',function() {
		...
		$("#form1")[0].action="<%=request.getContextPath()%>/<%=appName %>/smsstatus.action?";
		$("#form1").submit();					
	});	
