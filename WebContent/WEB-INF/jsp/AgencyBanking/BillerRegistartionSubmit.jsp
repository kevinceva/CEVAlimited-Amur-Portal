	  ...
	  .focusout(function() {  
	   ...
	   .blur(function() {
		   if($('#abbreviation').val().length > 0){
				...
				var queryString = "";	
				//alert(queryString);
				$.getJSON("billersCountJSONAct.action", queryString,function(data) { 
					var billerCount =data.responseJSON.BILLER_COUNT;
					//alert(billerCount);
					$('#billerCode').val($('#abbreviation').val().toUpperCase()+" - 000"+billerCount.toString().substring(0,6).replace(".",""));
				});
			  } else {
				  $('#billerCode').val('');
			  }
