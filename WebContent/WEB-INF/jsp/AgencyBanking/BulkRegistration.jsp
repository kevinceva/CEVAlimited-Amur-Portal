...
var billerrules = {
		rules : {
			...
			mobileNo : { required : true, number : true, minlength : 9 , maxlength : 9 },
...
$(document).ready(function(){   
	...
	$("#abbreviation")
	  .focusout(function() { 
		  if($('#abbreviation').val().length > 0 && $('#billerCode').val().length  == 0)
				$('#billerCode').val($('#abbreviation').val().toUpperCase()+"-"+(Math.random() * 100000).toString().substring(0,7).replace(".",""));
	  });
	  /* .blur(function() {
		  alert('test1')
		  if($('#code').val().length > 0)
				$('#code').val($('#abbreviation').val()+"-"+(Math.random() * 100000).toString().substring(0,7).replace(".",""));
	  }); */
	...
	$('#btn-submit').live('click',function() {  
		if($("#form1").valid()) {
			...
			$("#form1")[0].action="<%=request.getContextPath()%>/<%=appName %>/bulkPaymentRegistrationConf.action";
			$("#form1").submit();	
			return true;
		} else {
			return false;
	  ...
	  <div id="content" class="span10"> 
		    ...
		    <div> 
				<ul class="breadcrumb">
				  ...
				  <li><a href="viewOrganization.action?pid=130">E-Wallet</a><span class="divider"> &gt;&gt; </span> </li>
				...
				<table width="98%"  border="0" cellpadding="5" cellspacing="1" 
					class="table table-striped table-bordered bootstrap-datatable " > 
						 <tr> 
							...
							<td width="30%"><input type="text" name="name"  id="name" required=true  value="<s:property value='bulkBean.name' />"   />   </td>
						...
						<tr > 
							...
							<td> <input type="text" name="mobileNo"  id="mobileNo" value="<s:property value='bulkBean.mobileNo' />"  /> <span id="mobileNo_err" class="errmsg"></span></td> 
						...
						<tr >  
							...
							<td><input type="text" name="daysToDisburse"  id="daysToDisburse" value="<s:property value='bulkBean.daysToDisburse' />" required=true  />  </td>							
							<td><label for="Organization Type"><strong>Organization Type<font color="red">*</font></strong></label></td>
							<td> 
								<select id="organizationType" name="organizationType" class="chosen-select" style="width: 220px;">
